...
answer, but the container agent's `mcp__zipsa__ask` MCP call had a
10-min timeout. The call timed out, the agent re-asked, then gave up —
printing the full list of things it needed and exiting. The skill was
never built.

**Already mitigated (this PR).** Two changes shipped together reduce the
exposure: (a) the new feasibility & prerequisites step in
...
waits; and (b) `_MCP_TOOL_TIMEOUT_MS` was raised 10 min → 30 min
(`create.py:130`) so a genuinely slow human answer is less likely to
trip the timeout. The underlying fragility — the agent *exits* rather
than waits when a single `ask` finally does time out — still remains.

**Remaining fix for later.** Make the agent tolerate a timed-out `ask`
by retrying/polling instead of bailing (a timeout is "still waiting",
not "give up"). A larger timeout only pushes the cliff back; it doesn't
remove it.

**Why deferred.** The prerequisites step + larger timeout remove the
common trigger; the retry-on-timeout behavior is a separate change with
its own test surface. Pick up if a slow-HITL forge run still strands the
agent.
